...
import type { Demographics, SocialRobotInteraction } from '../types/experiment';
...
export function DemographicsPage({ initial, onContinue }: { initial?: Demographics; onContinue: (data: Demographics) => void }) {
  ...
  const [robot, setRobot] = useState<SocialRobotInteraction | undefined>(() =>
    normalizeRobotInteraction(initial?.previousSocialRobotInteraction),
  );
  ...
  return (
    <PageShell width="max-w-5xl">
      ...
      <div className="space-y-5">
        ...
        <QuestionCard title="Você já interagiu com robôs sociais?" description="Um robô social é uma máquina autônoma ou semi-autônoma projetada para interagir, se comunicar e conviver com seres humanos, utilizando comportamentos e normas sociais."><SocialRobotOptions value={robot} setValue={setRobot} /></QuestionCard>
...
function SocialRobotOptions({ value, setValue }: { value?: SocialRobotInteraction; setValue: (value: SocialRobotInteraction) => void }) {
  const options: SocialRobotInteraction[] = ['Sim', 'Não', 'Não sei'];
  return <div className="grid grid-cols-3 gap-3">{options.map((option) => <ExpressionOption key={option} name="robot" value={option} label={option} checked={value === option} onChange={() => setValue(option)} />)}</div>;
}

function normalizeRobotInteraction(value: unknown): SocialRobotInteraction | undefined {
  if (value === true) return 'Sim';
  if (value === false) return 'Não';
  if (value === 'Sim' || value === 'Não' || value === 'Não sei') return value;
  return undefined;
}
